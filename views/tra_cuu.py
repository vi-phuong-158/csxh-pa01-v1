...
def page_tra_cuu():
    ...
    conn = get_connection()
    try:
        # Construct SQL Filters (Optimization: Push filters to SQL)
        where_conditions = []
        query_params = []

        if filter_tinh != "Tất cả":
            where_conditions.append("dia_chi_tinh = ?")
            query_params.append(filter_tinh)

        if filter_gioi_tinh != "Tất cả":
            where_conditions.append("gioi_tinh = ?")
            query_params.append(filter_gioi_tinh)
        ...
        if search_query:
            ...
            # Further Optimized: Apply categorical filters in SQL first
            sql = f"SELECT * FROM doi_tuong {where_clause}"
            df_all = pd.read_sql_query(sql, conn, params=query_params)
            ...
            # Hiển thị với pagination (Apply filters)
            query = f"""
    ...
    # Lọc đặc thù (nếu có logic implement sau này)
    # Hiện tại các filter chính đã được push xuống SQL
    pass
