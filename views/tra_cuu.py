...
def page_tra_cuu():
    ...
    try:
        ...
        if search_query:
            # Lấy dữ liệu (đã lọc sơ bộ bằng SQL) để lọc tiếp bằng Python
            # Optimized by Bolt: Apply SQL filters BEFORE loading data into
            # Pandas
            query_sql = f"SELECT * FROM doi_tuong {where_sql}"
            df_all = pd.read_sql_query(query_sql, conn, params=params)
                ...
                if search_type in ["Tất cả", "Họ tên"]:
                    ...
                    if len(query_norm) >= 3:
                        ...
                        if remaining_indices.any():
                            ...
                            mask_hoten = mask_hoten | \
                                subsequence_matches.reindex(
            ...
            # Đếm tổng số records (với filters)
            count_query = f"SELECT COUNT(*) as total FROM doi_tuong {where_sql}"  # noqa: E501
            ...
            # Hiển thị với pagination (với filters)
            query = f"""
    ...
    # Các bộ lọc đã được áp dụng trong SQL, không cần lọc lại bằng Pandas
    ...
    if not df.empty:
        ...
        if 'cccd' in display_df.columns:
            ...
            display_df = display_df.rename(
                columns={
                    k: v for k, v in col_map.items() if k in display_df.columns
                }
            )
        ...
        with col_btn:
            if st.button("👁️ Xem hồ sơ", type="primary",
        ...
        file_name = f"danh_sach_doi_tuong_{timestamp}.csv"
        st.download_button(
            label="📥 Xuất Excel",
        ...
        st.info(
            "💡 Không có dữ liệu. Hãy thêm đối tượng mới trong "
            "phần **📝 Nhập liệu**."
        )
