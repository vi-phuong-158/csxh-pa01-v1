...
def page_tra_cuu():
    ...
    try:
        ...
        # Filter data at database level before loading into memory
        where_clauses = []
        params = []
        ...
        if search_query:
            # Lấy dữ liệu (đã lọc sơ bộ) để tìm kiếm mờ bằng Python
            # Optimized by Bolt: Push filters to SQL + Vectorized search
            query_sql = f"SELECT * FROM doi_tuong {where_str}"
            ...
            count_query = f"SELECT COUNT(*) as total FROM doi_tuong {where_str}"
    ...
    # Lọc đặc thù (Python side for now)
    # Note: Tinh/GioiTinh filters have been moved to SQL for performance
    if not df.empty:
