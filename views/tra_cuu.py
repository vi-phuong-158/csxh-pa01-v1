...
def page_tra_cuu():
    ...
    # Initialize reset key for search input
    if "search_reset_key" not in st.session_state:
        st.session_state.search_reset_key = 0
    ...
    with col1:
        ...
        search_query = st.text_input(
            ...
            key=f"search_input_{st.session_state.search_reset_key}"
        )
    ...
    if not df.empty:
        ...
        # Hiển thị Empty State thông minh
        if search_query:
            st.warning(
                f"⚠️ Không tìm thấy kết quả nào cho: **{search_query}**")

            # Button Clear Search
            if st.button("❌ Xóa bộ lọc & Tìm kiếm lại", type="secondary"):
                st.session_state.search_reset_key += 1
                st.rerun()
        else:
            st.info("💡 Chưa có dữ liệu hồ sơ trong hệ thống.")

            # Button Navigate to Input
            if st.button("➕ Thêm đối tượng mới", type="primary"):
