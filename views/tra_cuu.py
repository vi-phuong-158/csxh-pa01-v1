...
def page_tra_cuu():
    ...
    try:
        if search_query:
            ...
            # OPTIMIZATION: Use vectorized operations instead of iterrows for 5x-10x speedup
            mask = pd.Series(False, index=df_all.index)

            # 1. Check CCCD (Exact/Contains)
            if search_type in ["Tất cả", "CCCD"]:
                # Ensure cccd is string and check containment
                mask_cccd = df_all['cccd'].fillna("").astype(
                    str).str.contains(search_query, case=False, na=False)
                mask |= mask_cccd

            # 2. Check Họ tên (Fuzzy)
            if search_type in ["Tất cả", "Họ tên"]:
                # Pre-normalize query once
                n_query = normalize_string(search_query)

                # Vectorized normalization of 'ho_ten' column
                # normalize_string handles None/Empty, but good to ensure string
                normalized_names = df_all['ho_ten'].fillna(
                    "").astype(str).apply(normalize_string)

                # Containment check (Vectorized)
                mask_hoten = normalized_names.str.contains(
                    n_query, regex=False)

                # Subsequence check (for abbreviations/fuzzy) - only if query is long enough
                if len(n_query) >= 3:
                    def check_subsequence(text):
                        it = iter(text)
                        return all(char in it for char in n_query)

                    # Apply only where needed? simpler to apply to all for now or filter
                    mask_subseq = normalized_names.apply(check_subsequence)
                    mask_hoten |= mask_subseq
