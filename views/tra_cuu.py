...
def page_tra_cuu():
    ...
    try:
        if search_query:
            ...
            # OPTIMIZATION: Use Pandas Vectorized Operations instead of
            # iterrows
            ...
            if df_all.empty:
                ...
                # Tạo mask ban đầu là False
                mask = pd.Series(False, index=df_all.index)

                # Check CCCD (Exact/Contains)
                if search_type in ["Tất cả", "CCCD"]:
                    # Ensure CCCD is string and search
                    mask |= df_all['cccd'].astype(str).str.contains(
                        search_query, case=False, na=False)

                # Check Họ tên (Fuzzy)
                if search_type in ["Tất cả", "Họ tên"]:
                    # Chuẩn hóa cột họ tên (Vectorized map)
                    normalized_ho_ten = df_all['ho_ten'].map(normalize_string)

                    # 1. Exact contains (relaxed)
                    mask |= normalized_ho_ten.str.contains(
                        normalized_query, regex=False)

                    # 2. Subsequence match
                    if len(normalized_query) >= 3:
                        # Chỉ check những dòng chưa match để tiết kiệm
                        remaining_mask = ~mask
                        if remaining_mask.any():
                            def check_subsequence(text):
                                it = iter(text)
                                return all(
                                    char in it for char in normalized_query)

                            subseq_matches = normalized_ho_ten[remaining_mask].apply(
                                check_subsequence)
                            mask.loc[subseq_matches.index] = mask.loc[subseq_matches.index] | subseq_matches
