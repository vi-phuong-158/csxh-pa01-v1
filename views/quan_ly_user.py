...
def show_user_list():
    ...
    col2.metric("Super Admin", len([u for u in users if u['role'] == ROLE_SUPER_ADMIN]))
    col3.metric("User thường", len([u for u in users if u['role'] == ROLE_USER]))
    ...
    for user in users:
        with st.container():
            ...
            with col3:
                ...
                if last_login:
                    st.caption(f"Đăng nhập: {last_login[:10] if last_login else 'Chưa'}")
                else:
            ...
            with col5:
                ...
                if user['id'] != current_user.get('id'):
                    with st.popover("🗑️", help="Xóa tài khoản"):
                        st.markdown(f"Xóa **{user['username']}**?")
                        if st.button("Xác nhận", key=f"confirm_del_{user['id']}", type="primary"):
...
def show_create_user_form():
    ...
    with st.form("create_user_form"):
        ...
        must_change = st.checkbox("Yêu cầu đổi mật khẩu khi đăng nhập lần đầu", value=True)
