...
# Import services to replace local duplicates and enforce security
from services import (
    ...
    save_ho_so_dac_thu, validate_cccd
)
...
def validate_cccd_for_action(
        cccd: str, *required_fields) -> tuple[bool, str | None]:
    if not cccd:
    ...
    if not validate_cccd(cccd):
        return False, "⚠️ CCCD phải là 12 chữ số!"
...
# Removed local sanitize_filename, get_upload_folder, and save_* functions
# as they are now imported from services.py
...
def page_nhap_lieu():
    ...
    with tab1:
        ...
        with col1:
            ...
            avatar_file = st.file_uploader(
                "Tải lên ảnh chân dung", type=[
                    'png', 'jpg', 'jpeg'], key="main_avatar_uploader")
    ...
    with tab_nhan_than:
        ...
        if st.session_state.current_cccd:
            st.success(
                f"📌 Đang thêm thân nhân cho CCCD: **{st.session_state.current_cccd}**")
            current_cccd_nt = st.session_state.current_cccd
        else:
            st.warning(
                "⚠️ Vui lòng nhập và lưu thông tin cá nhân trước (Tab 1)")
        ...
        if current_cccd_nt and validate_cccd(
                current_cccd_nt) and check_cccd_exists(current_cccd_nt):
            ...
            if not df_nhan_than.empty:
                ...
                for idx, row in df_nhan_than.iterrows():
                    ...
                    with col_del:
                        if st.button(
                                "🗑️", key=f"nl_del_nt_{row['id']}", help=f"Xóa {row['ho_ten']}"):
        ...
        with col1:
            nt_ho_ten = st.text_input(
                ...
                placeholder="Nguyễn Văn A",
                key="nt_ho_ten")
            nt_cccd = st.text_input(
                "Số CCCD",
                placeholder="Nhập 12 số CCCD (nếu có)",
                key="nt_cccd")
        ...
        with col2:
            nt_phan_loai_nghe = st.selectbox(
                "Phân loại nghề nghiệp",
                PHAN_LOAI_NGHE_NGHIEP_OPTIONS,
                key="nt_phan_loai_nghe")
            nt_nghe_nghiep = st.text_input(
                "Chi tiết nghề nghiệp",
                placeholder="Ví dụ: Giáo viên THPT, Nông dân...",
                key="nt_nghe_nghiep")
        ...
        if st.button("💾 Lưu thân nhân", type="primary",
            ...
            if current_cccd_nt and nt_ho_ten:
                if not validate_cccd(current_cccd_nt):
                    st.error("⚠️ CCCD không hợp lệ (phải là 12 số)!")
                elif check_cccd_exists(current_cccd_nt):
                    ...
                    st.toast(
                        f"✅ Đã thêm {loai_quan_he}: {nt_ho_ten}", icon="🎉")
                    st.rerun()
                else:
    ...
    with tab_qt:
        ...
        with st.expander("➕ Thêm quá trình hoạt động", expanded=True):
            ...
            with col_qt_time:
                c1, c2 = st.columns(2)
                with c1:
                    qt_tu_nam = st.text_input(
                        "Từ năm", placeholder="2010", key="qt_tu_nam")
                with c2:
                    qt_den_nam = st.text_input(
                        "Đến năm", placeholder="2015", key="qt_den_nam")
            with col_qt_content:
            ...
            qt_ghi_chu = st.text_input(
                "Ghi chú",
                placeholder="Ghi chú thêm...",
                key="qt_ghi_chu")

            if st.button("💾 Lưu quá trình", type="primary",
                         use_container_width=True):
                if current_cccd_qt and qt_noi_dung:
                    if not validate_cccd(current_cccd_qt):
                        st.error("⚠️ CCCD không hợp lệ!")
                    elif check_cccd_exists(current_cccd_qt):
                        # Combine time fields
                        ...
                        save_qua_trinh_hoat_dong(
                            current_cccd_qt, qt_thoi_gian, qt_noi_dung, qt_ghi_chu)
        ...
        st.markdown("---")
        if current_cccd_qt and validate_cccd(
                current_cccd_qt) and check_cccd_exists(current_cccd_qt):
            ...
            if qt_list:
                ...
                for item in qt_list:
                    ...
                    with col_info:
                        st.markdown(
                            f"**{item['thoi_gian']}**: {item['noi_dung']}")
                    with col_del:
    ...
    with tab2:
        ...
        if st.session_state.current_cccd:
            st.success(
                f"📌 Đang thêm thông tin cho CCCD: **{st.session_state.current_cccd}**")
        ...
        with col1:
            ...
            if current_cccd and validate_cccd(
                    current_cccd) and check_cccd_exists(current_cccd):
                ...
                if not df_lien_he.empty:
                    with st.expander(f"📋 Danh sách liên hệ ({len(df_lien_he)})"):
                        for idx, row in df_lien_he.iterrows():
                            st.text(
                                f"- {row['loai_lien_he']}: {row['gia_tri']}")
            ...
            ghi_chu_lien_he = st.text_input(
                "Ghi chú", key="lien_he_note", placeholder="Ghi chú thêm...")

            if st.button("💾 Lưu liên hệ", use_container_width=True,
                         type="primary"):
                if current_cccd and gia_tri_lien_he:
                    if not validate_cccd(current_cccd):
                        st.error("⚠️ CCCD không hợp lệ!")
                    elif check_cccd_exists(current_cccd):
                        save_lien_he(
                            current_cccd,
                            loai_lien_he,
                            gia_tri_lien_he,
                            ghi_chu_lien_he)
                        st.toast(
                            f"✅ Đã thêm {loai_lien_he}: {gia_tri_lien_he}", icon="🎉")
                        st.rerun()
                    else:
                        st.error("⚠️ CCCD không tồn tại trong hệ thống!")
                else:
                ...
                if not df_tai_chinh.empty:
                    with st.expander(f"📋 Danh sách tài khoản ({len(df_tai_chinh)})"):
                        for idx, row in df_tai_chinh.iterrows():
                            st.text(
                                f"- {row['ngan_hang']}: {row['so_tai_khoan']}")

            ngan_hang = st.selectbox(
                "Ngân hàng",
                DANH_SACH_NGAN_HANG,
                key="ngan_hang_tab2")
            so_tai_khoan = st.text_input(
                "Số tài khoản", placeholder="1234567890")
            chu_tai_khoan = st.text_input(
                "Chủ tài khoản", placeholder="NGUYEN VAN A")
            ...
            if st.button("💾 Lưu tài khoản",
                ...
                if current_cccd and so_tai_khoan:
                    if not validate_cccd(current_cccd):
                        ...
                        save_tai_chinh(
                            current_cccd, ngan_hang, so_tai_khoan, chu_tai_khoan)
                        st.toast(
                            f"✅ Đã thêm TK {ngan_hang}: {so_tai_khoan}", icon="🎉")
                        st.rerun()
                    else:
                        st.error("⚠️ CCCD không tồn tại trong hệ thống!")
                else:
        ...
        with col3:
            ten_xe = st.text_input(
                "Tên xe", placeholder="Honda Vision, Toyota Vios...")

        if st.button("💾 Lưu phương tiện",
                     use_container_width=True, type="primary"):
    ...
    with tab3:
        ...
        if loai_hinh == "Hon_Nhan_NN":
            ...
            col1, col2 = st.columns(2)
            with col1:
                noi_dung_dict["ten_doi_tac"] = st.text_input(
                    "Họ tên đối tác", key="hn_ten")
                noi_dung_dict["quoc_tich"] = st.selectbox(
                    "Quốc tịch", DANH_SACH_QUOC_GIA, key="hn_qt")
            with col2:
                noi_dung_dict["so_ho_chieu"] = st.text_input(
                    "Số hộ chiếu", key="hn_hc")
            ...
            noi_dung_dict["ten_to_chuc"] = st.text_input(
                "Tên tổ chức NGO/FDI", key="lv_tc")
            col1, col2 = st.columns(2)
            with col1:
                noi_dung_dict["chuc_vu"] = st.text_input(
                    "Chức vụ", key="lv_cv")
            with col2:
                noi_dung_dict["thoi_gian"] = st.text_input(
                    "Thời gian làm việc", key="lv_tg")
            noi_dung_dict["dia_diem"] = st.text_input(
                "Địa điểm làm việc", key="lv_dd")
            ...
            with col1:
                ...
                noi_dung_dict["quoc_gia"] = st.selectbox(
                    "Quốc gia", DANH_SACH_QUOC_GIA, key="ht_qg")
            with col2:
                noi_dung_dict["thoi_gian_di"] = st.text_input(
                    "Thời gian đi", key="ht_tgd")
                noi_dung_dict["thoi_gian_ve"] = st.text_input(
                    "Thời gian về", key="ht_tgv")
            noi_dung_dict["nghe_sau_ve"] = st.text_input(
                "Nghề nghiệp sau khi về", key="ht_nghe")
            ...
            with col1:
                noi_dung_dict["quoc_gia"] = st.selectbox(
                    "Quốc gia", DANH_SACH_QUOC_GIA, key="vp_qg")
                noi_dung_dict["co_quan_bat"] = st.text_input(
                    "Cơ quan bắt giữ", key="vp_cq")
            with col2:
                vp_ngay = st.date_input(
                    ...
                    format="DD/MM/YYYY",
                    key="vp_tg")
                noi_dung_dict["thoi_gian"] = vp_ngay.strftime(
                    "%d/%m/%Y") if vp_ngay else ""
                noi_dung_dict["hinh_thuc_xu_ly"] = st.text_input(
                    "Hình thức xử lý", key="vp_ht")
            noi_dung_dict["noi_dung_vp"] = st.text_area(
                "Nội dung vi phạm", key="vp_nd", height=100)

        elif loai_hinh == "Xac_Minh":
            st.markdown("##### 🔍 Thông tin xác minh")
            col1, col2 = st.columns(2)
            with col1:
                noi_dung_dict["co_quan_xm"] = st.text_input(
                    "Cơ quan xác minh", key="xm_cq")
                xm_ngay = st.date_input(
                    "Ngày xác minh",
                    value=None,
                    format="DD/MM/YYYY",
                    key="xm_tg")
                noi_dung_dict["thoi_gian"] = xm_ngay.strftime(
            ...
            noi_dung_dict["noi_dung_xm"] = st.text_area(
                "Nội dung xác minh", key="xm_nd", height=100)

        ghi_chu_dac_thu = st.text_area(
            "Ghi chú thêm",
            placeholder="Ghi chú về hồ sơ đặc thù...",
            height=80)

        st.markdown("---")

        if st.button("💾 Lưu hồ sơ đặc thù", type="primary",
                     use_container_width=True):
            if current_cccd:
                if not validate_cccd(current_cccd):
                    st.error("⚠️ CCCD không hợp lệ!")
                    ...
                    if any(noi_dung_dict.values()):
                        save_ho_so_dac_thu(
                            current_cccd, loai_hinh, noi_dung_dict, ghi_chu_dac_thu)
                        st.toast(
                            f"✅ Đã lưu hồ sơ đặc thù: {
                                LOAI_HINH_DAC_THU[loai_hinh]}", icon="🎉")
                        st.rerun()
                    else:
                        st.warning("⚠️ Vui lòng nhập ít nhất một thông tin!")
                else:
                    st.error(
                        "⚠️ CCCD không tồn tại trong hệ thống! Vui lòng thêm thông tin cá nhân trước.")
            else:
        ...
        if current_cccd and validate_cccd(
            ...
            if not df_csxh.empty:
                ...
                st.caption(
                    "💡 Bạn có thể thêm nhiều loại hình CSXH cho cùng một người")

                for idx, row in df_csxh.iterrows():
                    loai_hinh_text = LOAI_HINH_DAC_THU.get(
                        row['loai_hinh'], row['loai_hinh'])
                    col_info, col_del = st.columns([5, 1])
                    with col_info:
                        try:
                            noi_dung = json.loads(
                            ...
                            info_preview = " | ".join(
                                [f"{v}" for v in list(noi_dung.values())[:2] if v])
                        except BaseException:
                            info_preview = ""
                        st.markdown(f"**📌 {loai_hinh_text}**: {info_preview}")
                    with col_del:
                        if st.button(
                                "🗑️", key=f"del_nl_csxh_{row['id']}", help=f"Xóa {loai_hinh_text}"):
                            if delete_ho_so_dac_thu(row['id']):
                                st.toast(f"✅ Đã xóa!", icon="🎉")
                                st.rerun()
            else:
                st.info(
                    "💡 Chọn loại hình và nhập thông tin để thêm hồ sơ đặc thù đầu tiên")
    ...
    with tab_tai_lieu:
        ...
        if st.session_state.current_cccd:
            st.success(
                f"📌 Đang thêm tài liệu cho CCCD: **{st.session_state.current_cccd}**")
        ...
        if current_cccd_tl and validate_cccd(
                current_cccd_tl) and check_cccd_exists(current_cccd_tl):
            ...
            if not df_tai_lieu.empty:
                ...
                for idx, row in df_tai_lieu.iterrows():
                    ...
                    with col_del:
                        if st.button(
                                "🗑️", key=f"del_tl_{row['id']}", help=f"Xóa {row['ten_file_goc']}"):
        ...
        if st.button("💾 Upload tài liệu", type="primary",
                     use_container_width=True):
            if current_cccd_tl and uploaded_file:
                if not validate_cccd(current_cccd_tl):
                    st.error("⚠️ CCCD không hợp lệ (phải là 12 số)!")
                elif check_cccd_exists(current_cccd_tl):
