    ...
    try:
        ...
        if avatar_file:
            try:
                ...
                cursor.execute(
                    "UPDATE doi_tuong SET anh_chan_dung = ? WHERE cccd = ?", (relative_path, data['cccd']))
...
def save_tai_lieu(cccd, uploaded_file, loai_tai_lieu, mo_ta=""):
    ...
    if file_ext not in ALLOWED_EXTENSIONS:
        return False, f"Định dạng không hỗ trợ! Chỉ chấp nhận: {', '.join([e for e in ALLOWED_EXTENSIONS])}"
...
def page_nhap_lieu():
    ...
    with tab1:
        ...
        with col1:
            ...
            avatar_file = st.file_uploader("Tải lên ảnh chân dung", type=[
    ...
    with tab_nhan_than:
                ...
                for idx, row in df_nhan_than.iterrows():
                    ...
                    with col_del:
                        with st.popover("🗑️", help=f"Xóa {row['ho_ten']}"):
                            st.markdown("Bạn có chắc chắn muốn xóa?")
                            if st.button("Xác nhận xóa", type="primary", key=f"confirm_del_nt_{row['id']}"):
        ...
        with col1:
            nt_ho_ten = st.text_input(
                "Họ và tên *", placeholder="Nguyễn Văn A", key="nt_ho_ten")
            nt_cccd = st.text_input(
                "Số CCCD", placeholder="Nhập 12 số CCCD (nếu có)", key="nt_cccd")
            nt_ngay_sinh = st.date_input("Ngày sinh", value=None, key="nt_ngay_sinh",
                                         format="DD/MM/YYYY", min_value=date(1900, 1, 1), max_value=date(2100, 12, 31))
        with col2:
            nt_phan_loai_nghe = st.selectbox(
                "Phân loại nghề nghiệp", PHAN_LOAI_NGHE_NGHIEP_OPTIONS, key="nt_phan_loai_nghe")
            nt_nghe_nghiep = st.text_input(
                "Chi tiết nghề nghiệp", placeholder="Ví dụ: Giáo viên THPT, Nông dân...", key="nt_nghe_nghiep")
            nt_noi_o = st.text_input(
                "Nơi ở hiện nay", placeholder="Địa chỉ hiện tại", key="nt_noi_o")

        nt_ghi_chu = st.text_input(
            "Ghi chú", placeholder="Ghi chú thêm...", key="nt_ghi_chu")
        ...
        with st.expander("➕ Thêm quá trình hoạt động", expanded=True):
            ...
            with col_qt_content:
                qt_noi_dung = st.text_area(
                    "Nội dung hoạt động", placeholder="Mô tả hoạt động...", height=100, key="qt_noi_dung")

            qt_ghi_chu = st.text_input(
                "Ghi chú", placeholder="Ghi chú thêm...", key="qt_ghi_chu")
        ...
        if current_cccd_qt and check_cccd_exists(current_cccd_qt):
            ...
            if qt_list:
                ...
                for item in qt_list:
                    ...
                    with col_del:
                        with st.popover("🗑️", help="Xóa quá trình hoạt động"):
                            st.markdown("Bạn có chắc chắn muốn xóa?")
                            if st.button("Xác nhận xóa", type="primary", key=f"confirm_del_qt_{item['id']}"):
    ...
    with tab2:
            ...
            if st.button("💾 Lưu liên hệ", use_container_width=True, type="primary"):
                if current_cccd and gia_tri_lien_he:
                    if check_cccd_exists(current_cccd):
                        save_lien_he(current_cccd, loai_lien_he,
                                     gia_tri_lien_he, ghi_chu_lien_he)
                        st.toast(
        ...
        with col2:
            ...
            ngan_hang = st.selectbox(
                "Ngân hàng", DANH_SACH_NGAN_HANG, key="ngan_hang_tab2")
            ...
            if st.button("💾 Lưu tài khoản", use_container_width=True, type="primary"):
                if current_cccd and so_tai_khoan:
                    if check_cccd_exists(current_cccd):
                        save_tai_chinh(current_cccd, ngan_hang,
    ...
    with tab3:
        ...
        if loai_hinh == "Hon_Nhan_NN":
            ...
            with col2:
                vp_ngay = st.date_input(
                    "Ngày vi phạm", value=None, format="DD/MM/YYYY", key="vp_tg")
            ...
            with col1:
                ...
                xm_ngay = st.date_input(
                    "Ngày xác minh", value=None, format="DD/MM/YYYY", key="xm_tg")
        ...
        ghi_chu_dac_thu = st.text_area(
            "Ghi chú thêm", placeholder="Ghi chú về hồ sơ đặc thù...", height=80)
            ...
            if current_cccd:
                if check_cccd_exists(current_cccd):
                    ...
                    if any(noi_dung_dict.values()):
                        ...
                        st.toast(
                            f"✅ Đã lưu hồ sơ đặc thù: {LOAI_HINH_DAC_THU[loai_hinh]}", icon="🎉")
                        st.rerun()
                    else:
                ...
                for idx, row in df_csxh.iterrows():
                    ...
                    with col_del:
                        with st.popover("🗑️", help=f"Xóa {loai_hinh_text}"):
                            st.markdown("Bạn có chắc chắn muốn xóa?")
                            if st.button("Xác nhận xóa", type="primary", key=f"confirm_del_csxh_{row['id']}"):
    ...
    with tab_tai_lieu:
                ...
                for idx, row in df_tai_lieu.iterrows():
                    ...
                    with col_del:
                        with st.popover("🗑️", help=f"Xóa {row['ten_file_goc']}"):
                            st.markdown("Bạn có chắc chắn muốn xóa?")
                            if st.button("Xác nhận xóa", type="primary", key=f"confirm_del_tl_{row['id']}"):
        ...
        with col2:
            mo_ta_tl = st.text_input(
                "Mô tả (tùy chọn)", placeholder="Mô tả nội dung file...", key="tl_mo_ta")
