...
from services import (
    ...
    get_upload_folder, update_doi_tuong, delete_doi_tuong,
    delete_lien_he, delete_tai_chinh, delete_phuong_tien,
    delete_nhan_than, delete_ho_so_dac_thu, delete_tai_lieu,
    validate_cccd
)
...
def get_nhan_than_by_cccd(cccd):
    ...
    df = pd.read_sql_query(
        "SELECT * FROM nhan_than WHERE cccd = ?",
        conn,
        params=(
            cccd,
        ))
...
def get_tai_lieu_by_cccd(cccd):
    conn = get_connection()
    df = pd.read_sql_query(
        "SELECT * FROM tai_lieu WHERE cccd = ? ORDER BY created_at DESC",
        conn,
        params=(
            cccd,
        ))
...
def page_profile_view(cccd):
    ...
    with col_header1:
        ...
        with st.expander("📷 Thay ảnh đại diện", expanded=False):
            ...
            if new_avatar_quick:
                if st.button("💾 Lưu ảnh", type="primary",
                             use_container_width=True, key="save_quick_avatar"):
                    try:
                        import time
                        ...
                        if upload_dir:
                            ...
                            file_ext = new_avatar_quick.name.split(
                                '.')[-1].lower()
    ...
    with col_header3:
        ...
        if st.button("🗑️ Xóa hồ sơ", type="secondary",
    ...
    if st.session_state.get('confirm_delete'):
        st.warning(
            f"⚠️ Bạn có chắc muốn xóa hồ sơ **{
                doi_tuong.get('ho_ten')}** không? Hành động này không thể hoàn tác!")
            ...
            with st.form("edit_form"):
                ...
                with col2:
                    edit_dia_chi_tinh = st.selectbox(
                        "Tỉnh/TP",
                        TINH_OPTIONS,
                        index=TINH_OPTIONS.index(
                            doi_tuong.get(
                                'dia_chi_tinh',
                                'Phú Thọ')) if doi_tuong.get('dia_chi_tinh') in TINH_OPTIONS else 0,
                    ...
                    edit_phan_loai = st.selectbox(
                        ...
                        index=PHAN_LOAI_NGHE_NGHIEP_OPTIONS.index(
                            doi_tuong.get(
                                'phan_loai_nghe_nghiep',
                                'Lao động tự do')) if doi_tuong.get('phan_loai_nghe_nghiep') in PHAN_LOAI_NGHE_NGHIEP_OPTIONS else 0,
                        key="edit_phan_loai"
                    )
    ...
    # ... (Rest of the file remains similar but deleted redundant save functions were imported)
    ...
    with tab_nt:
        ...
        if not df_nhan_than.empty:
            ...
            for idx, row in df_nhan_than.iterrows():
                ...
                with col_del:
                    if st.button(
                            "🗑️", key=f"del_nt_{row['id']}", help=f"Xóa {row['ho_ten']}"):
    ...
    # ... (Rest is same, calls imported functions)
    # I'll just keep the rest of the file structure but ensure I closed the file content correctly
    # ...
        ...
        if qt_list:
            ...
            for item in qt_list:
                with st.container():
                    ...
                    with col_del:
                        if st.button(
                                "🗑️", key=f"del_qt_pv_{item['id']}", help="Xóa mục này"):
    ...
    with tab2:
        ...
        if not df_lien_he.empty:
            for idx, row in df_lien_he.iterrows():
                ...
                with col_info:
                    ghi_chu_text = f" | 📝 {
                        row['ghi_chu']}" if row['ghi_chu'] else ""
                    st.markdown(
                        f"**{row['loai_lien_he']}**: {row['gia_tri']}{ghi_chu_text}")
                with col_del:
                    if st.button(
                            "🗑️", key=f"del_lh_{row['id']}", help=f"Xóa {row['loai_lien_he']}"):
        ...
        if not df_tai_chinh.empty:
            for idx, row in df_tai_chinh.iterrows():
                ...
                with col_del:
                    if st.button(
                            "🗑️", key=f"del_tc_{row['id']}", help=f"Xóa TK {row['ngan_hang']}"):
        ...
        with st.expander("➕ Thêm tài khoản ngân hàng", expanded=False):
            with st.form("add_tai_chinh_form"):
                ...
                if st.form_submit_button("💾 Lưu tài khoản", type="primary"):
                    if tc_so_tk:
                        save_tai_chinh(
                            cccd, tc_ngan_hang, tc_so_tk, tc_chu_tk, tc_ghi_chu)
        ...
        if not df_phuong_tien.empty:
            for idx, row in df_phuong_tien.iterrows():
                ...
                with col_info:
                    ten_xe = f" - {
                        row['ten_phuong_tien']}" if row['ten_phuong_tien'] else ""
                    ghi_chu_text = f" | 📝 {
                        row['ghi_chu']}" if row['ghi_chu'] else ""
                    st.markdown(
                ...
                with col_del:
                    if st.button(
                            "🗑️", key=f"del_pt_{row['id']}", help=f"Xóa {row['bien_kiem_soat']}"):
        ...
        if not df_dac_thu.empty:
            for idx, row in df_dac_thu.iterrows():
                ...
                with st.expander(f"📌 {loai_hinh_text}", expanded=True):
                    ...
                    with col_del:
                        if st.button(
                                "🗑️ Xóa", key=f"del_csxh_{row['id']}", help=f"Xóa hồ sơ {loai_hinh_text}"):
        ...
        with st.expander("➕ Thêm hồ sơ đặc thù mới", expanded=False):
            ...
            with st.form("add_csxh_profile_form"):
                ...
                if st.form_submit_button(
                        "💾 Lưu hồ sơ đặc thù", type="primary"):
                    # Kiểm tra có dữ liệu hợp lệ
                    has_data = any(
                        v for v in noi_dung.values() if v) or csxh_ghi_chu
                    if has_data:
                        save_ho_so_dac_thu(
                            cccd, csxh_loai, noi_dung, csxh_ghi_chu)
                        st.success(
                            f"✅ Đã thêm hồ sơ: {
                                LOAI_HINH_DAC_THU.get(
                                    csxh_loai, csxh_loai)}")
                        st.rerun()
                    else:
    ...
    with tab_tl:
        ...
        if not df_tai_lieu.empty:
            ...
            for idx, row in df_tai_lieu.iterrows():
                ...
                with col_del:
                    if st.button(
                            "🗑️", key=f"pv_del_tl_{row['id']}", help=f"Xóa {row['ten_file_goc']}"):
