                ...
                with st.container():
                    ...
                    with col_del:
                        with st.popover("🗑️", help="Xóa mục này"):
                            st.markdown("Bạn chắc chắn muốn xóa?")
                            if st.button("Xác nhận xóa", key=f"confirm_del_qt_pv_{item['id']}", type="primary"):
            ...
            for idx, row in df_lien_he.iterrows():
                ...
                with col_del:
                    with st.popover("🗑️", help=f"Xóa {row['loai_lien_he']}"):
                        st.markdown("Bạn chắc chắn muốn xóa?")
                        if st.button("Xác nhận xóa", key=f"confirm_del_lh_{row['id']}", type="primary"):
                            if delete_lien_he(row['id']):
                                st.toast(f"✅ Đã xóa liên hệ!")
                                st.rerun()
        else:
            ...
            for idx, row in df_tai_chinh.iterrows():
                ...
                with col_del:
                    with st.popover("🗑️", help=f"Xóa TK {row['ngan_hang']}"):
                        st.markdown("Bạn chắc chắn muốn xóa?")
                        if st.button("Xác nhận xóa", key=f"confirm_del_tc_{row['id']}", type="primary"):
                            if delete_tai_chinh(row['id']):
                                st.toast(f"✅ Đã xóa tài khoản!")
                                st.rerun()
        else:
            ...
            for idx, row in df_phuong_tien.iterrows():
                ...
                with col_del:
                    with st.popover("🗑️", help=f"Xóa {row['bien_kiem_soat']}"):
                        st.markdown("Bạn chắc chắn muốn xóa?")
                        if st.button("Xác nhận xóa", key=f"confirm_del_pt_{row['id']}", type="primary"):
                            if delete_phuong_tien(row['id']):
                                st.toast(f"✅ Đã xóa phương tiện!")
                                st.rerun()
        else:
            ...
            for idx, row in df_tai_lieu.iterrows():
                ...
                with col_del:
                    with st.popover("🗑️", help=f"Xóa {row['ten_file_goc']}"):
                        st.markdown("Bạn chắc chắn muốn xóa?")
                        if st.button("Xác nhận xóa", key=f"confirm_pv_del_tl_{row['id']}", type="primary"):
