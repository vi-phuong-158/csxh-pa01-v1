...
def validate_cccd(cccd: str) -> bool:
    """Check if CCCD is valid and safe (alphanumeric only)."""
    if not cccd:
        return False
    # Strictly alphanumeric to prevent path traversal
    return cccd.isalnum()


def get_upload_folder(cccd):
    """Lấy thư mục upload cho một CCCD (Safe)"""
    if not validate_cccd(cccd):
        raise ValueError("Invalid CCCD format")

    base_path = Path(__file__).parent / "uploads" / cccd
    # Extra safety: resolve and check
    base_path = base_path.resolve()
    uploads_root = (Path(__file__).parent / "uploads").resolve()

    if not str(base_path).startswith(str(uploads_root)):
