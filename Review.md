...
| Ưu tiên | Vấn đề | File |
|---------|--------|------|
| P2 | Dedup không nhất quán: `nhan_than`/`qua_trinh`/`dac_thu` không chống trùng khi upload lại file | `services/nhap_excel.py` |
| P2 | Dùng `fetch()` thuần (vi phạm [UI-1]) cho ECharts/đồ thị | `network/index.html`, `profile/_tab_mang_luoi.html`, `bao_cao_charts.js` |
| P2 | Document hệ quả đổi mật khẩu DB (KI-02) | GUI đổi mật khẩu |
