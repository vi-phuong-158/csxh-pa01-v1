...
class DoiTuong(Base):
    ...
    dia_chi_tinh: Mapped[Optional[str]] = mapped_column(
        String, default="Phú Thọ")
    ...
    # Relationships
    lien_he: Mapped[List["LienHe"]] = relationship(
        back_populates="doi_tuong", cascade="all, delete-orphan")
    tai_chinh: Mapped[List["TaiChinh"]] = relationship(
        back_populates="doi_tuong", cascade="all, delete-orphan")
    phuong_tien: Mapped[List["PhuongTien"]] = relationship(
        back_populates="doi_tuong", cascade="all, delete-orphan")
    nhan_than: Mapped[List["NhanThan"]] = relationship(
        back_populates="doi_tuong", cascade="all, delete-orphan")
    ho_so_dac_thu: Mapped[List["HoSoDacThu"]] = relationship(
        back_populates="doi_tuong", cascade="all, delete-orphan")
    tai_lieu: Mapped[List["TaiLieu"]] = relationship(
        back_populates="doi_tuong", cascade="all, delete-orphan")
    qua_trinh: Mapped[List["QuaTrinhHoatDong"]] = relationship(
        back_populates="doi_tuong", cascade="all, delete-orphan")
...
class LienHe(Base):
    ...
    cccd: Mapped[str] = mapped_column(
        ForeignKey("doi_tuong.cccd", ondelete="CASCADE"))
    # SDT, Email, Facebook...
...
class HoSoDacThu(Base):
    ...
    doi_tuong: Mapped["DoiTuong"] = relationship(
        back_populates="ho_so_dac_thu")
...
class QuanHeDoiTuong(Base):
    ...
    id: Mapped[int] = mapped_column(
        Integer, primary_key=True, autoincrement=True)
    cccd_1: Mapped[str] = mapped_column(
        ForeignKey("doi_tuong.cccd", ondelete="CASCADE"))
    cccd_2: Mapped[str] = mapped_column(
        ForeignKey("doi_tuong.cccd", ondelete="CASCADE"))
    loai_quan_he: Mapped[Optional[str]] = mapped_column(String)
    mo_ta: Mapped[Optional[str]] = mapped_column(Text)
    nguon_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey("nguon_du_lieu.id"))
