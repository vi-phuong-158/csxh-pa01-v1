...
# Quan hệ thân nhân
QUAN_HE = ["Bố đẻ", "Mẹ đẻ", "Vợ", "Chồng", "Con trai", "Con gái", "Anh trai", "Em gái", "Ông nội", "Bà nội"]

# Quốc gia
QUOC_GIA = ["Trung Quốc", "Hàn Quốc", "Nhật Bản", "Đài Loan", "Thái Lan", "Malaysia", "Singapore", "Mỹ", "Úc", "Đức"]
...
def generate_email(name):
    ...
    import unicodedata
    name_part = ''.join(c for c in unicodedata.normalize('NFD', name_part) if unicodedata.category(c) != 'Mn')
...
def generate_vehicle_name(loai_xe):
    """Tạo tên phương tiện"""
    cars = ["Toyota Vios", "Honda City", "Hyundai Accent", "Kia Morning", "Mazda 3", "VinFast Lux"]
    bikes = ["Honda Wave", "Honda SH", "Yamaha Exciter", "Suzuki Raider", "Honda Winner", "VinFast Klara"]
    if "Ô tô" in loai_xe:
...
def generate_test_data(num_records=1000):
    ...
    for i, cccd in enumerate(all_cccd):
        if random.random() < 0.40:  # 40% có tài khoản
            ...
            # Loại bỏ dấu
            import unicodedata
            ho_ten = ''.join(c for c in unicodedata.normalize('NFD', ho_ten) if unicodedata.category(c) != 'Mn')
    ...
    for i, cccd in enumerate(all_cccd):
        if random.random() < 0.50:  # 50% có quá trình
            ...
            birth_year = int(doi_tuong_data[i]["Ngày sinh (dd/mm/yyyy)"].split("/")[2])
    ...
    for cccd in all_cccd:
        if random.random() < 0.15:  # 15% có yếu tố đặc thù
            ...
            nam_ket_thuc = str(random.randint(int(nam_bat_dau), 2024)) if random.random() > 0.3 else ""
            
            if loai_hinh == "Hon_Nhan_NN":
                ten_nguoi = random.choice(["WANG Xiaoming", "KIM Min-jun", "TANAKA Yuki", "CHEN Wei", "LEE Jae-hyun"])
                ...
                companies = ["Samsung Electronics Vietnam", "LG Display", "Canon Vietnam", "Panasonic VN", "Intel Products Vietnam", "Honda Vietnam"]
                ho_so_dac_thu_data.append({
                    "CCCD (*)": cccd,
                    "Loại hình (*)": loai_hinh,
                    "Quốc tịch/Quốc gia": quoc_gia,
                ...
                truong = ["Đại học Tokyo", "Đại học Seoul", "Đại học Bắc Kinh", "MIT", "Đại học Melbourne"]
                ho_so_dac_thu_data.append({
                    "CCCD (*)": cccd,
                    "Loại hình (*)": loai_hinh,
                    "Quốc tịch/Quốc gia": quoc_gia,
                ...
                vi_pham = ["Cư trú bất hợp pháp", "Vi phạm lao động", "Vi phạm giao thông", "Ở quá hạn visa"]
                ho_so_dac_thu_data.append({
                    "CCCD (*)": cccd,
                    "Loại hình (*)": loai_hinh,
                    "Quốc tịch/Quốc gia": quoc_gia,
                ...
                co_quan = ["PA01", "Công an tỉnh", "Sở Công thương", "Bộ Công an"]
                ket_qua = random.choice(["Đủ điều kiện", "Không đủ điều kiện", "Đang xác minh"])
                ho_so_dac_thu_data.append({
                    "CCCD (*)": cccd,
                    "Loại hình (*)": loai_hinh,
                    "Quốc tịch/Quốc gia": "",
